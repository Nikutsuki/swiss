import type { DataFragment, MediaChunkPayload } from "../types";
import { deserializeMediaChunk, serializeMediaChunk } from "./mediaSerialization";
...
export class MediaDataChannelPipeline {
  private readonly maxMessageSize: number;
  private readonly channel: DataChannelLike;
  private readonly onChunkReceived: (payload: MediaChunkPayload) => void;
  private nextMessageId = 1;
  private readonly reassembly = new Map<number, ReassemblyEntry>();

  constructor(
    channel: DataChannelLike,
    onChunkReceived: (payload: MediaChunkPayload) => void,
  ...
  sendChunk(payload: MediaChunkPayload): void {
    this.assertChannelOpen();
    const encoded = serializeMediaChunk(payload);
    ...
    for (let fragmentIndex = 0; fragmentIndex < fragmentCount; fragmentIndex += 1) {
      ...
      this.channel.send(this.serializeFragment({
        kind: "fragment",
        messageId,
        fragmentIndex,
        fragmentCount,
  ...
  receiveBinary(data: ArrayBuffer | Uint8Array): void {
    ...
    if (bytes[0] === WHOLE_CHUNK_KIND) {
      this.onChunkReceived(deserializeMediaChunk(bytes.subarray(1)));
      return;
    }
    if (bytes[0] !== FRAGMENT_KIND) {
      throw new Error("Unknown data channel packet kind");
    }
    this.receiveFragment(this.deserializeFragment(bytes));
  }
  ...
  private receiveFragment(fragment: DataFragment): void {
    ...
    if (entry.receivedCount < entry.fragmentCount) return;
    ...
    this.onChunkReceived(deserializeMediaChunk(merged));
  }
  ...
  private deserializeFragment(bytes: Uint8Array): DataFragment {
    ...
    return { kind: "fragment", messageId, fragmentIndex, fragmentCount, payload };
  }
