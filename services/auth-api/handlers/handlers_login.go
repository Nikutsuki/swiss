...
func (h *Handler) finishLogin(w http.ResponseWriter, r *http.Request, preferCookieSession bool) {
	...
	applyAssertionFlagsForLegacyCredential(webAuthnCreds, dbCreds, parsed.RawID, parsed.Response.AuthenticatorData.Flags)
	...
	case len(loginScope) == 16:
		...
		if err != nil {
			log.Printf("WebAuthn passkey login validation failed for %s: %v", email, err)
	...
	case len(sessionData.UserID) > 0:
		...
		if err != nil {
			log.Printf("WebAuthn login validation failed for %s: %v", email, err)
			http.Error(w, "Cryptographic signature validation failed", http.StatusUnauthorized)
			return
		}
	...
	if err = h.db.UpdateCredentialFlags(ctx, database.UpdateCredentialFlagsParams{
		BackupEligible: credential.Flags.BackupEligible,
		BackupState:    credential.Flags.BackupState,
		CredentialID:   assertedCred.CredentialID,
	}); err != nil {
		log.Printf("Failed to update credential flags: %v", err)
	}
