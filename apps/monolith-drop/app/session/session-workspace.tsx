...
export default function SessionWorkspace() {
  ...
  const burnPct = useMemo(() => {
    ...
    const left = Math.max(0, end - (nowMs || start));
        ...
        <div className="grid w-full min-w-0 flex-1 grid-cols-1 gap-6 sm:gap-8 lg:grid-cols-12 lg:gap-10 xl:gap-12">
          <aside className="flex flex-col gap-6 sm:gap-8 lg:gap-10 lg:col-span-2">
            ...
            <section>
              ...
              {inviteUrl ? (
                <div className="space-y-3 break-all bg-(--surface-container-low) p-4 font-mono text-[12px] leading-relaxed text-(--security-emerald)">
                  {inviteUrl}
                  <div className="flex justify-center rounded-lg p-3">
                    <div className="bg-white p-3 sm:p-4 rounded-xl border border-white">
                      <QRCodeSVG
                        value={inviteUrl}
                        size={140}
                        level="M"
